...
export function generateResponse(question: string): string {
  ...
  // Advanced "how-to" responses
  if (q.includes('advanced configuration')) {
    return `To configure advanced settings in your CDP, follow these steps:
...
4. Save the changes and test the configuration to ensure it works as expected.

For detailed documentation, refer to your CDP's official guide.`;
  }

  if (q.includes('integration') && q.includes('api')) {
    return `To integrate your application with the CDP using the API, follow these steps:

1. Obtain your API key from the CDP dashboard.
2. Use the API documentation to understand the endpoints available.
3. Implement the API calls in your application using your preferred programming language.
4. Test the integration thoroughly to ensure data is flowing correctly.
